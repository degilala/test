...
class Env(object):
    ...
    def reset(self):
        ...
        if self.use_seg:
            self.fixed_seg_pve0 = items['fixed_seg_pve0']
            self.fixed_seg_pve1 = items['fixed_seg_pve1']
            self.fixed_seg_pve2 = items['fixed_seg_pve2']
            # self.labels = np.unique(self.fixed_seg)
            self.moving_seg_pve0 = utils.tensor(items['moving_seg_pve0']).to(self.device)
            self.moving_seg_pve1 = utils.tensor(items['moving_seg_pve1']).to(self.device)
            self.moving_seg_pve2 = utils.tensor(items['moving_seg_pve2']).to(self.device)
            # self.moving_seg = []
            # for i, lab in enumerate(self.labels):
            #     m_seg = (m_segs == lab)*255.
            #     self.moving_seg.append(m_seg)

            self.moved_seg_pve0 = copy.deepcopy(self.moving_seg_pve0)
            self.moved_seg_pve1 = copy.deepcopy(self.moving_seg_pve1)
            self.moved_seg_pve2 = copy.deepcopy(self.moving_seg_pve2)
    ...
    def score(self):
        if self.use_seg:
            ...
            #     dices.append(dice)
            #dices=[0,0,0]
            dices_pve0 = utils.dice(self.fixed_seg_pve0[0, 0]>0,utils.numpy_im(self.moved_seg_pve0, 1, device=self.device)>0)
            #dices_pve1 = utils.dice(self.fixed_seg_pve1[0, 0] > 0,
            #                        utils.numpy_im(self.moved_seg_pve1, 1, device=self.device) > 0)
            #dices_pve2 = utils.dice(self.fixed_seg_pve2[0, 0] > 0,
            #                        utils.numpy_im(self.moved_seg_pve2, 1, device=self.device) > 0)
            #dices[0]=dices_pve0
            #dices[1]=dices_pve1
            #dices[2]=dices_pve2
            return np.mean(dices_pve0)
        else:
    ...
    def step(self, field):
        ...
        self.moved_seg_pve0 = self.seg_stn(self.moving_seg_pve0,self.field)
        self.moved_seg_pve1 = self.seg_stn(self.moving_seg_pve1, self.field)
        self.moved_seg_pve2 = self.seg_stn(self.moving_seg_pve2, self.field)
