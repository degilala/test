...
class Encoder(nn.Module):
    ...
    def dist_multivar_normal(self, mu, logvar):#定义了一个多变量正态分布对象，给定一个均值张量mu和对角协方差矩阵对数张量logvar，该方法返回一个torch.distributions.MultivariateNormal对象。这里使用对角协方差矩阵是因为这个编码器生成的高维向量的各个分量之间是相互独立的。
