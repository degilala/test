...
class SPAC:
    ...
    def choose_action(self, state, test=False):
        #将 self.planner 和 self.actor 两个神经网络设为评估模式（eval()）
        self.planner.eval()
        self.actor.eval()
        dist, enc = self.planner(state)#使用 self.planner 对输入的状态进行处理，得到一个概率分布 dist 和一个编码器输出 enc。
        ...
        if not test:
            ...
            #如果test参数为False，则函数将field的值截断到-1和1之间。如果test参数为True，则函数将field的值截断到-3和3之间。
        else:
    ...
    def load_planner(self, model_path):
        self.planner.load_state_dict(torch.load(model_path))
        self.planner.eval()
